...
public class SearchNameSurname {
    public List<String> editNameSurname(List<String> fileName){
        ...
        for (String logItem : fileContent) {
            Matcher matcherName = attributePatternName.matcher(logItem);
            if (matcherName.find( )) {
                fileContent2.add(logItem.replaceAll(matcherName.group(1), connectNameSurname(Name, Surname)));
            }
            else fileContent2.add(logItem);
        }
        WriteXml writeXml = new WriteXml();
        writeXml.writeXmlFile(fileContent2);
        return fileContent;
    }
